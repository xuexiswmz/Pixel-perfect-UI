...
function renderBoxLayoutCss(sections) {
  if (!Array.isArray(sections) || sections.length === 0) return "";
  const blocks = [];

  for (const section of sections) {
    const box = section.box || (section.styles && section.styles.box);
    const positioning = section.positioning || (section.styles && section.styles.positioning);
    if (!box || positioning !== "absolute") continue;

    const sel = kebabCase(section.name);
    const rules = [];
    if (box.x !== undefined) rules.push(`  left: ${box.x}px;`);
    if (box.y !== undefined) rules.push(`  top: ${box.y}px;`);
    if (box.width) rules.push(`  width: ${box.width}px;`);
    if (box.height) rules.push(`  height: ${box.height}px;`);

    if (rules.length > 0) {
      blocks.push(`/* Box-model layout for ${section.name} */\n[data-section="${sel}"] {\n  position: absolute;\n${rules.join("\n")}\n}`);
    }
  }
...
function renderAllPrecisionCss(plan) {
  ...
  const boxLayout = renderBoxLayoutCss(plan.sections || []);
  return overrides + boxLayout;
}

module.exports = { pageStyles, renderPreciseOverrides, renderBoxLayoutCss, renderAllPrecisionCss };